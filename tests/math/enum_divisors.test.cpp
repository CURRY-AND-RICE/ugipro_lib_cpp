...
int main() {
    ...
    auto divisors1 = ugilib::enum_divisors(c);
    auto divisors2 = ugilib::enum_divisors_with_prime_factors(c, ugilib::prime_factorization(c));
    sort(all(divisors2));
    assert(divisors1 == divisors2);

    auto it_a = lower_bound(all(divisors1), a);
    auto it_b = lower_bound(all(divisors1), b);
    if (it_b != divisors1.end() && *it_b == b) it_b++;
